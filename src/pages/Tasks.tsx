    ...
    <div className="h-full flex flex-col space-y-4 md:space-y-6 max-w-7xl mx-auto px-4 py-8 md:p-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 flex-shrink-0">
        <div>
          ...
          <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-1 text-white">Action Items</h1>
          <p className="text-zinc-400 mt-2 font-medium text-sm">Track responsibilities, maintenance, and operational to-dos.</p>
        </div>
        <button 
      ...
      <div className="flex-shrink-0 flex flex-col sm:flex-row gap-4 mb-4">
         <div className="relative w-full sm:w-96">
          ...
          <div className="flex bg-[#111] p-1 rounded-xl border border-white/10 w-full sm:w-auto overflow-x-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 pb-6">
        {filteredTasks.map(task => {
          ...
          return (
            <div key={task.id} className="bg-[#111] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors flex flex-col">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-1.5 bg-black px-3 py-1.5 rounded-lg border border-white/5">
                  {getStatusIcon(task.status)}
                  <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-300">{task.status}</span>
                </div>
              </div>
              
              <h3 className="text-lg font-black text-white mb-2 leading-tight">{task.title}</h3>
              <p className="text-zinc-400 font-medium text-sm mb-6 flex-1">{task.description}</p>
              
              <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center font-black text-white text-xs">
                    {assignee?.name.charAt(0) || '?'}
                  </div>
                  <div>
                    <div className="font-bold text-white uppercase text-[10px] tracking-widest">{assignee?.name || 'Unassigned'}</div>
                ...
                <select 
                  ...
                  className="bg-black border border-white/10 rounded-lg text-white text-[10px] font-bold uppercase tracking-widest p-2 focus:outline-none focus:border-orange-500 appearance-none"
                >
        ...
        {filteredTasks.length === 0 && (
          <div className="col-span-full py-12 flex flex-col justify-center items-center text-zinc-500 border border-dashed border-white/5 rounded-2xl">
        ...
        <form onSubmit={handleCreateTask} className="space-y-5">
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full px-4 py-4 rounded-xl border border-white/10 bg-black text-white focus:outline-none focus:border-orange-500 transition-colors font-medium placeholder:text-zinc-600 min-h-[100px] resize-none"
